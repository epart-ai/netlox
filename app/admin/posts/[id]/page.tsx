            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-400">
                  작성자: {post.author_name} | 작성일: {new Date(post.created_at).toLocaleString('ko-KR')}
                  {post.updated_at && (
              ...
              <div className="space-y-4 rounded border border-slate-800 bg-slate-950/60 p-4">
                ...
                {contentLines.length > 0 ? (
                  contentLines.map((line) => (
                    <p key={line.id} className="whitespace-pre-wrap text-sm leading-6 text-slate-100">
                      {line.value}
                    </p>
                  ))
                ) : (
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-300">첨부 파일</h3>
                {attachments.length > 0 ? (
                  ...
                  <p className="mt-2 text-sm text-slate-400">첨부 파일이 없습니다.</p>
                )}
              </div>
            </div>
          ) : (
